...
class Log {
  ...
  static filenames(key) {
    // todo [akamel] can we use the history.txt file instead of klaw?
    return Promise
            .fromCallback((cb) => {
    ...
    return Promise
            ...
            .then((patch) => {
              ...
              if (_.isObject(meta)) {
                ...
                // todo [akamel] should be configurable per account/user
  ...
  static insert(data = {}, options = {}) {
    return Promise
            .try(() => {
              ...
              let a = Log.write_rec(key, data)
                , b = Index.add(key, data, options)
                ;
